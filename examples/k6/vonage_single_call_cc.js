/**
 * Vonage single-call smoke test
 *
 * setup()   → SIP REGISTER (UDP, digest auth, 180 s expiry)
 * default() → call VR extension 443361 once for 20 s using hard.wav, send BYE, log RTP/RTCP stats
...
const USERNAME  = __ENV.VONAGE_USERNAME_1;
const DOMAIN    = __ENV.VONAGE_DOMAIN_1;
const REGISTRAR = `sip:${DOMAIN}`;
const AOR       = `sip:${USERNAME}@${DOMAIN}`;
const PASSWORD  = __ENV.VONAGE_PASSWORD_1;
const CALLEE    = `sip:${__ENV.VONAGE_CALLEE_1}@${DOMAIN}`;
const DURATION  = '20s';
...
// ── setup: register once before VUs start ────────────────────────────────────
export function setup() {
  ...
  try {
    sip.register({
      ...
      password:   PASSWORD,
      expires:    180,
      // transport defaults to UDP — matches Vonage edge (port 5060)
    });
...
export default function (data) {
  ...
  console.log(`[VU] Calling ${CALLEE} for ${DURATION} …`);
...
// ── teardown: registration expires naturally (180 s) ─────────────────────────
